...
for k=1:p.Hp
    ...
    width = -.9;    
    normals = width*[0 -1;1 0]*left_unit_vector ; %%膨胀障碍物
    bineq(rows) = left_unit_vector' * (p.checkpoints(checkpoint_indices(k)).left+normals);
    
    
    rows = rows(end) + (1);
    Aineq(rows, idx_slack_lateral) = -1;
    Aineq(rows, idx_pos(k,:)) = -left_unit_vector';
    bineq(rows) = -left_unit_vector' * (p.checkpoints(checkpoint_indices(k)).right-normals);
...
%  r = 2;
% for k=2:p.Hp
%     Aineq(rows+k-1, idx_pos(k-1))=-[1;1] ;
%     Aineq(rows+k, idx_pos(k))=[1;1] ;
%     bineq(rows+1)=
% end
