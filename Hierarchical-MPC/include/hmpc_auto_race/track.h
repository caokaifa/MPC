...
public:
    ...
    Track(string file_name, float space) : space(space){
        ...
        for(std::vector<std::string> vec : dataList){
           ...
           // std::cout<<"-----wp.x----"<<wp.x<<" "<<wp.y<<std::endl;
    ...
    double findTheta(double x, double y, double theta_guess, bool global_search= false){
         ...
         * */
           //  wrapTheta(theta_guess);
            ...
            //用于计算转弯
            traj_ref_x=centerline.at(min_ind).x;
            traj_ref_y=centerline.at(min_ind).y;
            ...
            double distance_centerline=sqrt(pow((p - p0).norm(),2)-pow(projection,2));
//            if (theta>length){ theta -= length;}
//            if (theta<0){ theta += length;}

            return theta-0.3*distance_centerline;
    }
